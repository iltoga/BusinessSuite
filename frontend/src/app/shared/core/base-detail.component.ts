...
export abstract class BaseDetailComponent<T> implements OnInit {
  ...
  protected onDelete(): void {
    ...
    const confirmMessage = this.config.messages?.deleteConfirm?.(this.item()!) ??
    ...
    this.deleteItem(this.itemId).subscribe({
      next: () => {
        const successMessage = this.config.messages?.deleteSuccess ??
          `${this.config.entityLabel} deleted successfully`;
      ...
      error: (error) => {
        ...
        const errorMessage = this.config.messages?.deleteError ??
  ...
  handleGlobalKeydown(event: KeyboardEvent): void {
    ...
    const canDelete = this.config.enableDelete !== false && 
  ...
  protected restoreNavigationState(): void {
    ...
    this.returnUrl.set(typeof state.returnUrl === 'string' && state.returnUrl.startsWith('/') 
      ? state.returnUrl 
      : null);
