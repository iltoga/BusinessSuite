...
class BenchmarkDataSafetyPropertyTests(TransactionTestCase):
    """
    ...
    def test_benchmark_does_not_modify_database(self, num_users, num_queries):
        ...
        self.assertEqual(
            initial_users,
            final_users,
            "Benchmark modified User records"
        )
        
        # Verify counts unchanged
        self.assertEqual(
            User.objects.count(),
            self.initial_user_count,
            "Benchmark changed User count"
        )
    ...
    def test_benchmark_uses_separate_redis_db(self, num_users):
        ...
        self.assertEqual(
            ...
            "Benchmark modified production cache version"
        )
    ...
    def test_benchmark_transaction_rollback(self):
        ...
        self.assertEqual(
            initial_count,
            final_count,
            "Benchmark created new database records"
        )
    ...
    def test_benchmark_read_only_queries(self, model_name):
        ...
        self.assertEqual(
            initial_data,
            final_data,
            f"Benchmark modified {model_name} data"
        )
...
class BenchmarkSafetyUnitTests(TestCase):
    ...
    def test_benchmark_max_users_limit(self):
        ...
        with self.assertRaises(Exception) as context:
            call_command(
                "benchmark_cache",
                "--users", "10000",  # Exceeds default limit
                "--queries", "10",
                stdout=out,
                stderr=err,
            )
    ...
    def test_benchmark_max_queries_limit(self):
        ...
        with self.assertRaises(Exception) as context:
            call_command(
                "benchmark_cache",
                "--users", "10",
                "--queries", "100000",  # Exceeds default limit
                stdout=out,
                stderr=err,
            )
