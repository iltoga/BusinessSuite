...
- Switched non-test PgQueuer broker from `SqlPgQueuer` to `RedisPgQueuer` (test mode remains Sqlite/SqlPgQueuer).
...
- Updated `.env.example` with Redis/PgQueuer env variables and removed `MEMCACHED_HOST`.
- Updated Python dependencies to remove `pymemcache` and add `django-redis` + `redis`.
- Added tests for Redis cache/PgQueuer settings and extended invoice sequence cache-increment coverage.
