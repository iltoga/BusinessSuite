# Redis Migration (Cache + Dramatiq Broker)
...
- Dramatiq broker in non-test environments: SQL-backed Dramatiq -> Redis-backed Dramatiq
...
In local development, only infrastructure runs in containers. Backend, Dramatiq worker, and frontend run on host.
...
2. Run app processes on host:
   ...
   - worker: `cd backend && DJANGO_SETTINGS_MODULE=business_suite.settings ../.venv/bin/python manage.py run_dramatiq`
